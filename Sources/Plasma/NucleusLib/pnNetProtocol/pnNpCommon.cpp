...
{
    ST::utf16_buffer strBuf = hsSTStringToUTF16LE(str);
    // Add one extra char16_t for the string terminator, which must be included in the data.
    uint32_t bytes = (strBuf.size() + 1) * sizeof(char16_t);
    IWriteValue(bytes, buffer);

    size_t endPos = buffer->size();
    buffer->resize(endPos + bytes);
    memcpy(buffer->data() + endPos, strBuf.c_str(), bytes);
}
...
{
    ST::utf16_buffer utf16 = hsSTStringToUTF16LE(value);
